...
func (m *Memory) SendAddress(address uint16) {
	m.address = address
	first, addr := splitaddr(address)
	m.bit1, m.last14 = first, addr
...
func (c *Computer) ClockTick() {
	...
	// skip clock tick on ROM: it never writes anyways

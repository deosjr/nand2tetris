...
func (b *BuiltinCPU) evalALU(c [7]bit) (out uint16, zr, ng bit) {
	x := toBit16(b.d.Out())
...
func (b *BuiltinCPU) ClockTick() {
	isC, c, dest, jump := b.decode()
	outM, isZero, isNeg := b.evalALU(c)
	b.outM = outM
...
func (b *BarrelShiftCPU) evalALU(shift bit, c [7]bit) (out uint16, zr, ng bit) {
	x := toBit16(b.d.Out())
	y := Mux16(toBit16(b.a.Out()), toBit16(b.inM), c[0])
	var o [16]bit
	o, zr, ng = Alu(x, y, c[1], c[2], c[3], c[4], c[5], c[6])
...
func (b *BarrelShiftCPU) ClockTick() {
	isC, pcrl, shift, comp, dest, jump := b.decode()
	outM, isZero, isNeg := b.evalALU(shift, comp)
	b.outM = outM
	isPos := And(Not(isNeg), Not(isZero))
	outA := b.a.Out()
	pc := b.PC()
